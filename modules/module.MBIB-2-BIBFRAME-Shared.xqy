...
declare variable $mbshared:simple-properties:= (
  <properties>
    ...
    <node domain="title"   property="titleQualifier"     tag="730" sfcodes="p" >title qualifier</node>
...
{
  (:let $biblink:=
    fn:concat($workID, fn:normalize-space(fn:string($marcxml/marcxml:controlfield[@tag eq "001"]))
    ):)
  let $derivedFrom :=
  ...
  let $edited:=
    if ($marcxml/marcxml:controlfield[@tag="005"]) then
      fn:concat(
        fn:substring(($marcxml/marcxml:controlfield[@tag="005"]),1,4),
        "-",
        fn:substring(($marcxml/marcxml:controlfield[@tag="005"]),5,2),
        "-",
        fn:substring(($marcxml/marcxml:controlfield[@tag="005"]),7,2),
        "T",
        fn:substring(($marcxml/marcxml:controlfield[@tag="005"]),9,2),
        ":",
        fn:substring(($marcxml/marcxml:controlfield[@tag="005"]),11,2))
    else
      ()
  let $changed:=
    ( element bf:generationProcess {fn:concat("DLC transform-tool:", $mbshared:last-edit)},
      if ($edited) then
  ...
  ) as element ()
{
  ...
  let $freq:=
    for $s in $d/../marcxml:datafield[@tag="321"][fn:position()=$position - 1]
  ...
  ) as element ()
{
  ...
  let $instance-title :=
    ( element bf:titleValue {
        if (fn:contains($d/../marcxml:datafield[@tag="245"]/marcxml:subfield[@code="a"],"=")) then
  ...
  ) as element ()
{
          ...
          element bf:relatedInstance {
            element bf:Instance {
              ...
              (element bf:edition {marc2bfutils:clean-string($e/marcxml:subfield[@code="a"])},
                if ($e/marcxml:subfield[@code="b"]) then
  ...
  let $edition-880 :=
    for $ed in $d/../marcxml:datafield[@tag = "250"][marcxml:subfield[@code="a"]]
  ...
  let $publication:=
    ...
    else if (fn:matches($d/@tag, "(261|262)")) then
      mbshared:generate-26x-pub($d)
    else ()
    ...
    (
      for $i in $d/../marcxml:datafield[@tag eq "034"]/marcxml:subfield[@code eq "a"]
        return element bf:cartographicScale {
          if (fn:string($i)="a") then
            "Linear scale"
          else if (fn:string($i)="b") then
            "Angular scale"
          else if (fn:string($i)="z") then
            "Other scale type"
          else "invalid"
        }
                ,
  ...
  let $instance-types:=
    for $i in fn:distinct-values($instance-types)
      return
        element rdf:type {
          attribute rdf:resource { fn:concat("http://bibframe.org/vocab/" ,$i)}
        }

  let $issuance:=
    if (fn:matches($leader7,"(a|c|d)")) then
      "monographic"
    else if ($leader7="b") then
      "continuing"
    else if ($leader7="m" and fn:matches($leader19,"(a|b|c)")) then
      "multipart monograph"
    else if ($leader7='m' and $leader19=' ') then
      "single unit"
    else if ($leader7='i') then
      "integrating resource"
    else if ($leader7='s') then
      "serial"
    else ()
  let $issuance :=
    if ($issuance) then
  ...
  let $physform :=
    for $s in $d/../marcxml:datafield[@tag = "020"]/marcxml:subfield[@code="q"]
  ...
  let $color:=
    if ($d/ancestor::marcxml:record/marcxml:controlfield[@tag="007"]) then
      for $c in $d/ancestor::marcxml:record/marcxml:controlfield[@tag="007"][fn:matches(fn:substring(marcxml:record/marcxml:controlfield[@tag="007"],1,1) ,"(a|c|d|g|h|k|m|v)")]
        let $colorcode:=
          if (fn:substring($c,1,1)="h") then
             fn:substring($c,9,1)
          else
             fn:substring($c,4,1)

        return
          if ($colorcode= "a") then "One color"
  ...
  let $aspect :=
    if ($d/ancestor::marcxml:record/marcxml:controlfield[@tag="007"]) then
      for $a in $d/ancestor::marcxml:record/marcxml:controlfield[@tag="007"][ fn:substring(text(),1,1)="m"]
        ...
        return
          if ($aspectcode= "a" ) then "Standard sound aperture (reduced frame)"
  ...
  let $instance-identifiers := ( mbshared:generate-identifiers($d/ancestor::marcxml:record,"instance") )
  ...
  let $i504:=
    for $i in $d/../marcxml:datafield[@tag="504"]
      let $b :=
        if ($i/marcxml:subfield[@code="b"]) then
    ...
    return
      for $d in $marcxml/marcxml:datafield[@tag='880'][fn:matches(marcxml:subfield[@code="6"],"^[0-9]{3}-00$")]
        ...
        for $node880 in $nodes[fn:string(@tag)= $tag-to-convert]
          let $return-codes:=
            if ($node880/@sfcodes) then
              fn:string($node880/@sfcodes)
            else
              "a"
...
{
  let $identifiers := $mbshared:simple-properties//node[@domain=$domain][@group="identifiers"]
    ...
    for $this-tag in $marcxml/marcxml:datafield[fn:matches( $taglist,fn:string(@tag) )]
      return
        for $id in $identifiers[fn:not(@ind1)][@domain=$domain][@tag=$this-tag/@tag] (:all but 024 and 028:)
          ...
          let $cancels :=
            for $sf in $this-tag/marcxml:subfield[fn:matches(@code,"(m|y|z)")]
              return element
                { fn:concat("bf:",fn:string($id/@property)) }
                { mbshared:handle-cancels($this-tag, $sf, fn:string($id/@property)) }

          return
            (:need to construct blank node if there's no uri or there are qualifiers/assigners :)
            if (fn:not($id/@uri)
                or  $this-tag/marcxml:subfield[fn:matches(@code,"(b|q|2)")]
                or  $this-tag[@tag="037"][marcxml:subfield[@code="c"]]
                  ...
                  element bf:Identifier {
                    ...
                    if ($this-tag/marcxml:subfield[@code="a"]) then
                      if ( $this-tag/marcxml:subfield[@code="a"][fn:matches(text(),"^.+\(.+\).+$")]) then
                        ...
                        return
                          element bf:identifierValue { fn:substring($val,1, fn:string-length($val)-1) }
                      else
  ...
  let $id024-028 :=
    for $this-tag at $x in $marcxml/marcxml:datafield[fn:matches(@tag,"(024|028)")][marcxml:subfield[@code="a" or @code="z"]]
      ...
      return
        if ($this-id) then
          (: if there are any 024/028s on this record in this domain (work/instance):)
          let $scheme :=
            if ($this-tag/@ind1="7") then
              (:use the contents of $2 for the name: :)
          ...
          return (
            if ( fn:not($this-tag/marcxml:subfield[@code="z"])
                  and ( $this-tag/marcxml:subfield[@code="a"] and
                    ( fn:contains(fn:string($this-tag/marcxml:subfield[@code="c"]), "(") or
                      $this-tag/marcxml:subfield[@code="q" or @code="b" or @code="2"]
                    )
                    or fn:not($this-id/@uri)
                    or $scheme="unspecified"
                  )
                ) then
              let $value:=
                if ($this-tag/marcxml:subfield[@code="a"] or $this-tag/marcxml:subfield[@code="d"]) then
...
:)
declare function mbshared:handle-system-number (
    $sys-num as element(marcxml:subfield)
  )
{
...
:)
declare function mbshared:handle-cancels (
    $this-tag,
    $sf,
    $scheme
  )
  ...
  let $translationOf :=
    if ($d/marcxml:subfield[@code="l"]) then
      (for $s in  $d/marcxml:subfield[@code="l"]
        ...
        (:let $lang:= $marc2bfutils:lang-xwalk/language[@language-name=marc2bfutils:chopPunctuation($s,".")]/iso6392[1]
          return
            if ($lang!="")
            then
              element bf:language {
